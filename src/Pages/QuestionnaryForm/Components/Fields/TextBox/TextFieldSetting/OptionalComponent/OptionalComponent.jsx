...
function OptionsComponent({
  ...
  const selectedServiceValue = useSelector(
    (state) => state?.questionnaryForm.selectedServiceValue
  );
  const [toggleStates, setToggleStates] = useState({});
  const [activeTab, setActiveTab] = useState("attributeData");
  const [isServiceRecordDropdownOpen, setServiceRecordDropdownOpen] =
    useState(false);
  const [isAttributeDropdownOpen, setIsAttributeDropdownOpen] = useState(false);
  const [isQuestionnariesDropdownOpen, setQuestionnariesDropdownOpen] =
    useState(false);
    ...
    const getAssetData = async () => {
      ...
      try {
        let response = await getAPI(
          `${import.meta.env.VITE_API_BASE_URL}asset_types/${
  ...
  const fetchQuestionnaireList = async (services_type) => {
    ...
    try {
      const response = await getAPI(
        `questionnaires${objectToQueryString(query)}`
      );
  ...
  const fetchQuestionList = async (selectedQuestionnaireId) => {
    try {
      ...
      const response = await getAPI(
        `questionnaires/list/${encodeURIComponent(selectedQuestionnaireId)}`
      );
  ...
  const handleQuesList = (questionList, componentType) => {
    ...
    const filteredList = questionList.filter(
      (item) => item.component_type === componentType
    );
    ...
    return filteredList.map((item) => {
      ...
      const componentTypeLabel =
        componentTypeLabels[componentType] || "Unknown Component";
  ...
  useEffect(() => {
    ...
    if (questionSettings) {
      setToggleStates({
        "Load from previously entered data":
          questionSettings.load_from_previous || false,
  ...
  const ToggleSwitch = ({ label, onChange, checked, testId }) => (
    <div className="status custom-toggle-switch flex items-center justify-between mt-2">
      <p className="text-base font-normal text-[#000000] mr-4">{label}</p>
    ...
    <div className="mt-7 w-[97%]">
      ...
      {options.includes("Load from previously entered data") && (
        <ToggleSwitch
          ...
          onChange={
            formStatus === "Draft"
              ? () => handleToggleClick("Load from previously entered data")
              : null
          }
        />
      )}
          ...
          <div className="mt-4">
            <div className="flex justify-between border-b border-gray-300 mb-2">
              <p
                ...
                className={`font-semibold text-base cursor-pointer ${
                  activeTab === "attributeData"
                    ? "text-black border-b-2 border-[#000000] pb-2"
                    : "text-[#9FACB9]"
                }`}
                onClick={
                  formStatus === "Draft"
                    ? () => handleTabClick("attributeData")
                    : null
                }
              >
                Attribute Data
              ...
              (isLoadingAsset ? (
                ...
                <InfinateDropdown
                  ...
                  selectedOption={attributes.find(
                    (option) =>
                      option.value === fieldSettingParameters.attribute_data_lfp
                  )}
            ...
            {activeTab === "serviceRecord" && (
              <InfinateDropdown
                ...
                selectedOption={serviceRecordOptions.find(
                  (option) =>
                    option.label === fieldSettingParameters.service_record_lfp
                )}
                handleOptionClick={
                  formStatus === "Draft" ? handleServiceClick : null
                }
                isDropdownOpen={
                  formStatus === "Draft" ? isServiceRecordDropdownOpen : false
                }
                setDropdownOpen={(isOpen) => {
                  if (formStatus === "Draft") {
              ...
              (fieldSettingParameters.service_record_lfp ? (
                isLoadingQuestionnaires ? (
                  ...
                  <InfinateDropdown
                    ...
                    labeltestID="service-record"
                    selectedOption={handleQuestionnarieList.find(
                      (option) =>
                        option.value ===
                        fieldSettingParameters.questionnaire_name_lfp
                    )}
                    handleOptionClick={
                      formStatus === "Draft" ? handleQuestionnaryClick : null
                    }
                    isDropdownOpen={
                      formStatus === "Draft"
                        ? isQuestionnariesDropdownOpen
                        : false
                    }
                    setDropdownOpen={(isOpen) => {
                      if (formStatus === "Draft") {
                ...
                showError && (
                  <ErrorMessage error={"No questionnaire is available"} />
                )
              ))}
            {activeTab !== "attributeData" &&
              (fieldSettingParameters.questionnaire_name_lfp &&
              handleQuesList(
                questionList,
                fieldSettingParameters?.componentType
              )?.length !== 0 ? (
                isLoadingQuestion ? (
                  <BeatLoader
                    color="#000"
                  ...
                  <InfinateDropdown
                    ...
                    handleOptionClick={
                      formStatus === "Draft" ? handleQuesClick : null
                    }
                    isDropdownOpen={
                      formStatus === "Draft" ? isQuesDropdownOpen : false
                    }
                    setDropdownOpen={(isOpen) => {
                      if (formStatus === "Draft") {
                ...
                <ErrorMessage
                  error={
                    "No questions available for the selected questionnaire"
                  }
                />
              ) : null)}
          </div>
      ...
      {options
        ...
        .map((option) => (
          <ToggleSwitch
            ...
            testId={option}
            onChange={
              formStatus === "Draft" ? () => handleToggleClick(option) : null
            }
          />
        ))}
    </div>
