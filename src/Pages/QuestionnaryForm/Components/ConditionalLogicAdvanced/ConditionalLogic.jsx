...
import {
  generateElseBlockString,
  generateTernaryOperation,
  generateThenActionString,
} from "../../../../CommonMethods/ComplianceBasicEditorLogicBuilder";
...
function ConditionalLogic({
  ...
  const allSectionDetails = useSelector(
    (state) => state?.allsectiondetails?.allSectionDetails
  );
  ...
  const selectedComponent = useSelector(
    (state) => state?.questionnaryForm?.selectedComponent
  );
  ...
  const fieldSettingParams = useSelector(
    (state) => state.fieldSettingParams.currentData
  );
  const complianceLogicCondition = useSelector(
    (state) => state.fieldSettingParams.conditions
  );
  const conditionalLogicData = useSelector(
    (state) => state.fieldSettingParams.editorToggle
  );
  ...
  const fileMethods = ["()"];
  const notRequiredConditions = ["toUpperCase()", "toLowerCase()", "trim()"]
        ...
        page.questions?.forEach((question) => {
          if (
            ...
            ![
              ...
              "gpsfield",
              "displayfield",
            ].includes(question?.component_type)
          ) {
  ...
  useEffect(() => {
    ...
    if (condition_logic?.includes(":")) {
      ...
      condition_logic =
        parts.map((part) => part.trim()).join(" else if ") +
        " else " +
        lastPart.trim();
    }
  ...
  const handleInputField = (event, sections) => {
    ...
    if (value[cursorPosition - 1] === ".") {
      if (
        selectedFieldType ===
        "textboxfield, choiceboxfield, assetLocationfield, floorPlanfield, signaturefield, gpsfield, displayfield"
      ) {
        ...
        const matchedQuestion = datetimefieldQuestions.find((question) =>
          questionMatches?.some(
            (match) =>
              match?.split(".")[2]?.replaceAll("_", " ").trim() ===
        ...
        if (matchedQuestion) {
          ...
          console.warn(
            "No matching question found for the selectedQuestionId."
          );
          setShowMethodSuggestions(false);
        }
        ...
        const wordToSearch = value.slice(
          startOfWord,
          cursorPosition + endOfWord
        );
  ...
  const handleClickToInsert = (textToInsert, isMethod, componentType) => {
    ...
    if (textarea) {
      ...
      const charBeforeCursor =
        cursorPosition > 0 ? textarea.value[cursorPosition - 1] : "";
      let newText;
  ...
  function getDetails(path, data) {
    ...
    const matchingSection = data?.find(
    ...
    const matchingPage = matchingSection.pages.find(
    ...
    const matchingQuestion = matchingPage.questions.find(
  ...
  const parseLogicExpression = (expression) => {
    ...
    const convertTimestampToDate = (timestamp) => {
      const date =
        timestamp.split("/")[0].length === 1 ? `0${timestamp}` : timestamp;
      return date;
    };
    // Helper function to parse date conditions
    const parseDateCondition = (condition) => {
      const pattern =
        /formatDateWithOffset\('([^']+)',\s*(\d+),\s*([\w\s.]+)\)/;
      const match = condition?.match(pattern);
      if (!match) {
        return null;
      }
      const [_, timestamp, offsetDays, question_name] = match;
      const question = getDetails(
        question_name.trim(),
        allSectionDetails.sections
      );
      ...
      const conditions = group.split("&&").map((condition) => {
        ...
        if (matches) {
          ...
          if (question?.component_type === "dateTimefield") {
            ...
            if (condition_logic === "<")
              condition_logic = "date is before today";
            else if (condition_logic === ">=" || condition_logic === "=>")
              condition_logic = "date is after or equal to today";
            else if (condition_logic === "<=" || condition_logic === "=<")
              condition_logic = "date is before or equal to today";
            else if (condition_logic === ">")
              condition_logic = "date is after today";
          ...
          if (
            ["photofield", "videofield", "filefield"].includes(
            ...
            if (
              ...
              condition_logic = "has no files";
            else if (condition_logic === ">=" || condition_logic === "=>")
              condition_logic = "has atleast one file";
            else if (condition_logic === "===" || condition_logic === "==")
              condition_logic = "number of file is";
          ...
          if (negate) {
            if (condition_logic.includes("includes")) {
              ...
              if (condition_logic === "===" || condition_logic === "==")
                condition_logic = "equals";
              else if (condition_logic === "!=")
                condition_logic = "not equal to";
              else if (condition_logic === "<") condition_logic = "smaller";
              else if (condition_logic === ">") condition_logic = "larger";
              else if (condition_logic === "<=")
                condition_logic = "smaller or equal";
              else if (condition_logic === ">=")
                condition_logic = "larger or equal";
            }
          } else {
            // Convert logical operators to corresponding values in conditions
            if (condition_logic === "===" || condition_logic === "==")
              ...
              condition_logic = "larger or equal";
            else if (condition_logic.includes("includes"))
              condition_logic = "includes";
          }
    ...
    const conditionGroups = expression
      .split("||")
      .map((group) => trimParentheses(group));
    ...
    const totalConditions = parsedConditions.reduce(
    ...
    const fetchData = async () => {
      if (complianceState) {
        try {
          ...
          if (selectedLogic) {
            const transformedContent = defaultContentConverter(
              selectedLogic.default_content
            );
    ...
    const findSelectedQuestion = () => {
      ...
      if (sectionConditionLogicId) {
        const section = sectionsData.find(
          (section) => section.section_id === sectionConditionLogicId
        );
        ...
        const section = sectionsData.find((section) =>
          section.section_id.includes(sectionId)
        );
        const page = section?.pages.find((page) =>
          page.page_id.includes(pageConditionLogicId)
        );
  ...
  const handleSave = async () => {
    ...
    let sectionId =
      selectedQuestionId.split("_")[0].length > 1
        ? selectedQuestionId.split("_")[0]
        : selectedQuestionId.split("_")[1];
    if (sectionConditionLogicId) {
    ...
    try {
      ...
      evalInputValue = evalInputValue
        .replaceAll(/ACTIONS?\s*\+=\s*"(.*?)"/g, `ACTIONS.push('$1')`)
        .replaceAll("if", "")
        .replaceAll("if", " ");
      ...
      if (evalInputValue.includes("getMonth")) {
        ...
        if (monthValueMatch) {
          ...
          setConditions;
          setError(
            "Invalid format. Please use the format `getMonth() === value`."
          );
          return;
        }
      }
      ...
      if (evalInputValue.includes("getDay")) {
        ...
        const dayValueMatch = evalInputValue.match(
          /getDay\(\)\s*(===|!==|>=|<=|>|<)\s*"(Sunday|Monday|Tuesday|Wednesday|Thursday|Friday|Saturday)"/i
        );
        // Check if a valid match was found
        if (dayValueMatch) {
          ...
          if (!validDays.includes(dayValue)) {
            setError(
              "Invalid day. Please enter a valid day from Sunday to Saturday."
            );
            return;
          }
          ...
          setError(
            'Invalid format. Please use the format `getDay() === "Day"`.'
          );
          return;
        }
      }
      if (evalInputValue.includes("getFullYear")) {
        ...
        const yearValueMatch = evalInputValue.match(
          /getFullYear\(\)\s*(===|!==|>=|<=|>|<)\s*(\d{4,})/
        );
        ...
        if (yearValueMatch) {
          ...
          setError(
            "Invalid format. Please use the format `getFullYear() === <year>`."
          );
          return;
        }
      }
      if (evalInputValue.includes("getTime")) {
        ...
        const timeMatches = evalInputValue.match(
          /getTime\(\)\s*(===|!==|>=|<=|>|<)\s*"(\d{2}:\d{2}:\d{2})"/g
        );

        if (timeMatches) {
          for (const timeMatch of timeMatches) {
            const [, operator, timeValue] = timeMatch.match(
              /getTime\(\)\s*(===|!==|>=|<=|>|<)\s*"(\d{2}:\d{2}:\d{2})"/
            );
            ...
            if (!timePattern.test(timeValue)) {
              setError(
                `Invalid time: "${timeValue}". Please enter a valid time in 'hh:mm:ss' format`
              );
              return;
            }
          }
          ...
          setError(
            'Invalid format. Please use the format `getTime() === "hh:mm:ss"`.'
          );
          return;
        }
      }
      ...
      if (evalInputValue.includes("getHours")) {
        ...
        if (timeMatches) {
          for (const timeMatch of timeMatches) {
            const matchParts = timeMatch.match(
              /getHours\(\)\s*(===|!==|>=|<=|>|<)\s*(\d{1,2})/
            );
            ...
            if (matchParts && matchParts.length === 3) {
              ...
              if (isNaN(hoursValue) || hoursValue < 0 || hoursValue > 23) {
                setError(
                  `Invalid hours: "${hours}". The value must be a valid number between 00 and 23.`
                );
                return;
              }
            } else {
              setError(
                "Unexpected format error while processing getHours() expressions."
              );
              return;
            }
          }
          ...
          setError(
            "Invalid format. Please use the format `getHours() === hh` where hh is a two-digit number."
          );
          return;
        }
      }
        ...
        if (timeMatches) {
          for (const timeMatch of timeMatches) {
            ...
            if (parseInt(minutes) < 0 || parseInt(minutes) > 59) {
              setError(
                `Invalid minutes: "${minutes}". The value must be a valid number between 00 and 59.`
              );
              return;
            }
          }
          ...
          setError(
            "Invalid format. Please use the format `getMinutes() === mm` where mm is a two-digit number."
          );
          return;
        }
      }
      if (evalInputValue.includes("getMilliseconds")) {
        ...
        const timeMatches = evalInputValue.match(
          /getMilliseconds\(\)\s*(===|!==|>=|<=|>|<)\s*(\d{3,})/g
        );

        if (timeMatches) {
          for (const timeMatch of timeMatches) {
            const [, operator, milliseconds] = timeMatch.match(
              /getMilliseconds\(\)\s*(===|!==|>=|<=|>|<)\s*(\d{3,})/
            );
            ...
            if (millisecondsValue < 0 || millisecondsValue > 999) {
              setError(
                `Invalid milliseconds: "${milliseconds}". The value must be between 000 and 999.`
              );
              return;
            }
          }
          ...
          setError(
            "Invalid format. Please use the format `getMilliseconds() === nnn` where nnn is a three-digit number."
          );
          return;
        }
      }
      ...
      if (
        ...
        selectedComponent === "dateTimefield" &&
        evalInputValue.includes("setDate")
      ) {
        let invalid = DateValidator(evalInputValue);
        if (invalid) {
          if (invalid) {
            handleError(
              `Error in ${invalid.join(
                ", "
      ...
      evalInputValue = Object.keys(questionWithUuid).reduce(
        (logic, questionName) => {
          // Escape all special regex characters
          let escapedQuestionName = questionName.replace(
            /[-[\]{}()*+?.,\\^$|#\s/~`!@#%^&_=:"';<>]/g,
            "\\$&"
          );
          return logic
            .replace(
              new RegExp(escapedQuestionName, "g"),
              questionWithUuid[questionName].replace(/-/g, "_")
            )
            .trim();
        },
        evalInputValue
      ...
      if (isDefaultLogic || complianceState) {
        switch (selectedComponent) {
          ...
          case "textboxfield":
            if (typeof result !== "string") {
              handleError(
                "The evaluated result is not a string. Only expects a string."
              );
              return;
            }
            break;
          case "numberfield":
            ...
            // Check if the type is 'float'
              ...
              if (!/^\d+(\.\d+)?$/.test(result)) {
                handleError(
                  "The evaluated result should be a valid float (number and dot)."
                );
                return;
              }
            } else {
        ...
        switch (selectedComponent) {
          ...
          case "tagScanfield":
            if (typeof result !== "string") {
              handleError(
                "The evaluated result is not a string. The field type expects a string."
              );
              return;
            }
            break;
  ...
  function splitAndValidate(expression) {
    ...
    const addDaysValidator =
      /^new Date\(new Date\((sections\.[\w\.]+)\)\.setDate\(new Date\(\1\)\.getDate\(\) [+-] \d+\)\)\.toLocaleDateString\("en-GB"\)\s*(==|!=|===|!==|<|>|<=|>=)\s*"\d{2}\/\d{2}\/\d{4}"$/;
    ...
    parts.forEach((part, index) => {
      ...
      const containsTypeMethod = typeMethods.some((method) =>
        part.includes(method)
      );
      ...
      if (part.includes("includes(")) {
        ...
        if (!result) {
          error.push(
            `Error in the ${part}: missing values inside the function`
          );
        }
      }
      ...
      // Check if it's a date type
        ...
        if (dateMatch) {
          ...
          if (
            value !== "Today" &&
            expression.includes("setDate") &&
            !validDateRegex.test(value)
          ) {
            errors.push(
              `Error in expression: "${value}" is not a valid date. Use 'dd/mm/yyyy' or 'Today'.`
            );
          }
        } else {
          errors.push(
            `Error in expression: Date field is missing or incorrectly formatted.`
          );
        }
      }
  ...
  const showInputValue = (key) => {
    ...
    let arr = [
      ...
      "date is before or equal to today",
      "date is after today",
      ...
      for (let i = 0; i < conditions.length; i++) {
        ...
        if (conditions[i].elseIfBlocks) {
          for (let k = 0; k < conditions[i].elseIfBlocks.length; k++) {
            for (
              let l = 0;
              l < conditions[i].elseIfBlocks[k].conditions.length;
              l++
            ) {
              const elseIfCondition =
                conditions[i].elseIfBlocks[k].conditions[l];
  ...
  const getComplianceLogic = (condition) => {
    ...
    const getConditionValue = (item) => {
      ...
      switch (item.condition_logic) {
        ...
        case "equals":
          if (
            item.condition_type === "choiceboxfield" ||
            item.condition_type === "numberfield"
          ) {
            resultExpression = `${item.question_name} === ${getValue(
              item.value,
              item.condition_type
            )}`;
          } else {
        ...
        case "number of file is":
          resultExpression = `${item.question_name}.length == ${getValue(
            item.value,
            item.condition_type
          )}`;
          break;
  ...
  const getFinalComplianceLogic = () => {
    ...
    finalString +=
      conditions[0]?.conditions[0]?.question_name !== ""
        ? "if (" + getComplianceLogic(conditions[0]?.conditions) + ")"
        : "";
    if (conditions[0].thenAction) {
      finalString +=
        " ? " +
        generateThenActionString(conditions[0]?.thenAction) +
        `${conditions[0]?.elseIfBlocks ? "" : " : "}`;
    }
    if (conditions[0]?.elseIfBlocks) {
      ...
      conditions[0]?.elseIfBlocks?.map((outerItem) => {
        ...
        if (outerItem?.thenActions) {
          finalString +=
            " ? " + generateThenActionString(outerItem?.thenActions[0]) + " : ";
        }
      });
    }
  ...
  function getReplacedComplianceLogic(conditions) {
    ...
    condition_logic = condition_logic.replace(
      /ACTIONS\.push\(['"](.*?)['"]\)/g,
      "ACTIONS = '$1'"
    );
  ...
  useEffect(() => {
    if (!complianceState && !isDefaultLogic) {
      let condition_logic = buildConditionExpression(
        conditions,
        combinedArray,
        (isAdvance = true)
      );
      ...
      try {
        ...
        if (condition_logic && condition_logic.includes(":")) {
          ...
          condition_logic =
            transformedParts.join(" else if ") + " else " + lastPart.trim();
        }

        const replacedLogic = replaceUUIDs(condition_logic, questionWithUuid);
        const defaultLogic = replaceUUIDs(
          complianceLogic[0].default_content,
          questionWithUuid
        );
        const finalLogic =
          replacedLogic || defaultContentConverter(defaultLogic);
  ...
  const handleSaveBasicEditor = () => {
    ...
    if (complianceState) {
      ...
      compliance_logic = Object.keys(questionWithUuid).reduce(
        (logic, questionName) => {
          // Escape all special regex characters
          let escapedQuestionName = questionName.replace(
            /[-[\]{}()*+?.,\\^$|#\s/~`!@#%^&_=:"';<>]/g,
    ...
    if (!complianceState) {
      condition_logic = Object.keys(questionWithUuid).reduce(
        (logic, questionName) => {
          // Escape all special regex characters
          let escapedQuestionName = questionName.replace(
            /[-[\]{}()*+?.,\\^$|#\s/~`!@#%^&_=:"';<>]/g,
      ...
      if (sectionConditionLogicId) {
        ...
        if (section) {
          ...
          compliance_logic = parseLogicExpression(
            replaceUUIDs(section?.section_conditional_logic, questionWithUuid)
          );
        } else {
          console.error(
            "Section not found for the given sectionConditionLogicId"
          );
        }
      } else if (pageConditionLogicId) {
        let pageFound = false;
        ...
        sectionsData.forEach((section) => {
          const page = section.pages?.find(
            (page) => page.page_id === pageConditionLogicId
          );
          ...
          if (page) {
            ...
            compliance_logic = parseLogicExpression(
              replaceUUIDs(page.page_conditional_logic, questionWithUuid)
            );
          }
        });
  ...
  return (
    <>
      <div className="bg-[#3931313b] w-full h-screen absolute top-0 flex flex-col items-center justify-center z-[999]">
        <div
          ref={modalRef}
          className="!w-[80%] h-[90%] mx-auto bg-white rounded-[14px] relative p-[18px] overflow-hidden"
        >
                    ...
                    {!isDefaultLogic && (
                      <div className="flex gap-5 items-end">
                        <button
                          ...
                          className={
                            tab === "advance"
                              ? "text-lg text-[#9FACB9] font-semibold px-[1px] border-b-2 border-white cursor-pointer"
                              : "text-[#2B333B] font-semibold px-[1px] border-b-2 border-[#2B333B] text-lg cursor-pointer"
                          }
                        >
                          Basic Editor
                        </button>
                        <p
                          data-testId="advance-editor-tab"
