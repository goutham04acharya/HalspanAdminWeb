Feature: Halspan - Admin - Variety of options available for each Field Type
    ...
    - Attribute data – If the field is to display current attribute data and/or collect asset
    attribute data then it must have this field checked.
    ...
    - default content populates the field with entered value and
    is used as value if form is submitted.
    - Remember allowed (Yes/No) – this allows a user to set a value as their own personal
    default, which is entered into this field in future Questionnaires.
