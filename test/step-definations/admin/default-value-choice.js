...
When('I click the add default value button', async function () {
    await new Promise(resolve => setTimeout(resolve, 950));
    await driver.wait(until.elementLocated(By.css(`[data-testid="default-value"]`))).click();
});

Then('I should see the default value advanced editor for choice field', async function () {
    await new Promise((resolve) => setTimeout(resolve, 750));
    await driver.wait(until.elementLocated(By.xpath('//*[text()="Default Value"]')));
});

When('I enter the incorrect default value conditional logic for choice field', async function () {
    await new Promise(resolve => setTimeout(resolve, 750));
...
When('I select the question from the default value suggestions for choice field', async function () {
    let bool = true;
    let suggestions = ['Section_1.Page_1.Sample_Choice_Label_Name'];
    let i = 0;

    // eslint-disable-next-line max-len
    await driver.wait(until.elementLocated(By.css(`[data-testid="conditional-logic-text"]`))).sendKeys('if (Section_1.Page_1.Sample_Choice_Label_Name.includes("Roopesh") AND ', Key.RETURN);
    await new Promise(resolve => setTimeout(resolve, 750));
    while (bool) {
...
When('I enter the default value correct conditional logic for choice field', async function () {
    await new Promise(resolve => setTimeout(resolve, 750));
    const default_value = `if (Section_1.Page_1.Sample_Choice_Label_Name === "India") then "INR" else "USD"`;
